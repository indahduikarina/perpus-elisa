            ...
            <tbody>
                ...
                <tr class="border-t hover:bg-pink-50 transition">
                    ...
                    <td class="px-4 py-2 space-x-2">
                        <button
                            onclick="openEditModal('{{ $item->idbuku }}', '{{ $item->judul }}', '{{ $item->pengarang }}', '{{ $item->penerbit }}', '{{ $item->tahun_terbit }}')"
...
<div id="modalTambah" class="fixed inset-0 z-50 hidden bg-black bg-opacity-50 flex items-center justify-center">
    <div
        class="bg-white w-[90%] sm:max-w-xl p-6 rounded-lg shadow-lg relative border-2 border-pink-300 max-h-screen overflow-y-auto"
    >
        ...
        <form action="{{ route('buku.store') }}" method="POST" class="space-y-4">
            @csrf
            <div>
                <label class="block text-sm font-medium text-pink-800 mb-1">ID Buku</label>
                <input
                    type="text"
                    name="idbuku"
                    class="w-full px-4 py-2 border border-pink-300 rounded-lg focus:ring-pink-400"
                    required
                >
            ...
            <div>
                <label class="block text-sm font-medium text-pink-800 mb-1">Judul</label>
                <input
                    type="text"
                    name="judul"
                    class="w-full px-4 py-2 border border-pink-300 rounded-lg focus:ring-pink-400"
            ...
            <div>
                <label class="block text-sm font-medium text-pink-800 mb-1">Pengarang</label>
                <input
                    type="text"
                    name="pengarang"
                    class="w-full px-4 py-2 border border-pink-300 rounded-lg focus:ring-pink-400"
            ...
            <div>
                <label class="block text-sm font-medium text-pink-800 mb-1">Penerbit</label>
                <input
                    type="text"
                    name="penerbit"
                    class="w-full px-4 py-2 border border-pink-300 rounded-lg focus:ring-pink-400"
            ...
            <div>
                <label class="block text-sm font-medium text-pink-800 mb-1">Tahun Terbit</label>
                <input
                    type="number"
                    name="tahun_terbit"
                    class="w-full px-4 py-2 border border-pink-300 rounded-lg focus:ring-pink-400"
                    required
                >
                @error('tahun_terbit')<p class="text-red-500 text-sm mt-1">{{ $message }}</p>@enderror
            </div>
...
<script>
    ...
    function openEditModal(id, judul, pengarang, penerbit, tahun) {
